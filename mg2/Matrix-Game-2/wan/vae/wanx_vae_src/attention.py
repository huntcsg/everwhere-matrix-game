...
try:
    ...
    # kernel, so it must not be reported as FlashAttention-2. clip.py -- the
    # visual context encoder this VAE wrapper owns -- calls flash_attention()
    # directly with version=2, so this is the first thing a job hits.
...
def _sdpa_attention(
    ...
    own SDPA branch, but clip.py calls flash_attention directly, so the
...
def flash_attention(
    ...
    if not (FLASH_ATTN_2_AVAILABLE or FLASH_ATTN_3_AVAILABLE):
        # LOCAL PATCH: no flash-attn kernel in this image. clip.py calls this
        # function directly, so it never reaches attention()'s SDPA branch.
